            ...
            <div className="surface-glow" style={{ padding: '2rem', borderRadius: '16px', position: 'relative', overflow: 'hidden' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    ...
                    <div className="mode-toggle" style={{ display: 'flex', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', padding: '4px' }}>
                        <button
                            className={`btn-sm ${uploadMode === 'standard' ? 'active' : ''}`}
                            onClick={() => { setUploadMode('standard'); setStep(1); }}
                            style={{ background: uploadMode === 'standard' ? 'var(--primary)' : 'transparent', border: 'none', color: uploadMode === 'standard' ? '#000' : '#fff', padding: '6px 24px', borderRadius: '6px', cursor: 'pointer' }}
                        >Standard Upload</button>
                        <button
                            className={`btn-sm ${uploadMode === 'bulk' ? 'active' : ''}`}
                            onClick={() => { setUploadMode('bulk'); setStep(1); }}
                            style={{ background: uploadMode === 'bulk' ? 'var(--primary)' : 'transparent', border: 'none', color: uploadMode === 'bulk' ? '#000' : '#fff', padding: '6px 24px', borderRadius: '6px', cursor: 'pointer' }}
                        >Bulk (ZIP)</button>
                    </div>
                </div>
